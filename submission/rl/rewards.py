...
class RewardCalculator:
    ...
    def _launch_bonus(self, total_ships: int, per_launch: list = None) -> float:
        """Per-launch reward: simple linear bonus for every ship sent.

        Micro-launch penalties are no longer needed — the offset scheme with
        pruning (``_allocate_ships_offset``) guarantees every launch meets
        at least ``target_ships + 1`` for enemies or 1 for neutrals, so no
        ship is wasted on an unwinnable attack.

        Not-launching (0 ships) is NOT penalised here — ``_no_action_penalty``
        handles prolonged inaction.
        """
        ...
        return sum(ships * self.launch_bonus_scale for ships in per_launch)
